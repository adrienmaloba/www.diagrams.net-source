...
Select a swimlane and press ``Alt+Enter`` to duplicate it, or ``Delete`` to remove it. Add and remove swimlanes to and from pools using the _Table_ tools in the _Arrange_ tab of the format panel, or drop a new swimlane shape into the pool.
