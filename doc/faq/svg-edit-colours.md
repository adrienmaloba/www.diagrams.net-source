...
To disbale support for separate dark mode colours, add ``color-scheme: light;`` in the style attribute of the SVG element or use ``darkMode=0`` in the cell style. (Note that dark mode for SVG images is not yet supported in Safari, it will use the light colours in both light and dark mode.)
